...
export class AppViewModel {
    ...
    public onComponentLoaded = (chartComponent: ChartViewModel) => {
        console.log('avm: onComponentLoaded');
        ...
        if (index >= 0) {
            ...
            chartComponent.init(layout.w, layout.h, layout.v);
        } else {
            throw new Error('Loaded chart component is not found');
        }
    }
    ...
    private getChartLayout(chartIndex: number): ILayout {
        ...
        if (chartIndex === 0) {
            return this.mode === 1
                ...
                : { w: this.frameSize.width / 2 , h: frameHeight / 2 - 2, v: true };
        } else {
            return this.mode === 1
                ? { w: this.frameSize.width / 2, h: frameHeight / 2 - 2, v: false }
                : { w: this.frameSize.width / 2 , h: frameHeight / 2 - 2, v: true };
        }
    }
}
