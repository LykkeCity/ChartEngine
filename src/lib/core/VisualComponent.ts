...
export abstract class VisualComponent { //implements IMouseHandler {
    ...
    private forEachAggregator(
        ...
        let index = directOrder ? 0 : this.children.length - 1;
        while ((directOrder && index < this.children.length - 1)
               || (!directOrder && index >= 0)) {
            if (!this.children[index].forEachAggregator(delegate, directOrder, offset)) {
